...
all_dates = pd.date_range(start='2024-02-01', end='2024-02-21')
...
def sort_first_n_days(n, cur_df):
    ...
    #DATE (DAY) VALIDATION PART
    unique_dates = filtered_df['Time'].dt.date.unique()
    missing_dates = [date for date in all_dates if date.date() not in unique_dates]
    for mis_date in missing_dates:
        date_time_range = pd.to_datetime(mis_date)
        ...
        same_day_names = filtered_df[(filtered_df['WD'] == mis_date.day_name())]
        random_append_day = same_day_names[same_day_names['Time'].dt.date == random.choice(same_day_names['Time'].dt.date.unique())]
        groups = []
        for number, group in random_append_day.groupby('Number'):
            group['Time'] = time_range
            random_append_day.loc[random_append_day['Number'] == number, :] = group.values

        filtered_df = pd.concat([filtered_df, random_append_day], ignore_index=True)
    #--------------------


    mask = filtered_df['WD'].isin(weekdays) #or weekends
    filtered_df = filtered_df[mask].sort_values(by=['Number', 'Time'])
